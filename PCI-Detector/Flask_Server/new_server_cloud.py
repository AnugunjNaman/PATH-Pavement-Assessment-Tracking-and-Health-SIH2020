...
def img_inference(img_path):
    with detection_graph.as_default():
        with tf.Session(graph=detection_graph) as sess:
            ...
            # image = np.ascontiguousarray(PIL.Image.open(img_path).convert('RGB'))
            ...
            im_op = cv2.resize(image_np,(600,600),interpolation=cv2.INTER_LINEAR)
            im_final = cv2.cvtColor(im_op,cv2.COLOR_BGR2RGB)
            cv2.imwrite(op_name,im_final)
   ...
   if request.method == 'POST':
    ...
    #   model = models.convert_model(model)
      ...
      if response.status_code == 200:
            ...
            path_img, pci = img_inference('{}/imgs/sample_png.png'.format(PATH))
