...
const RoadDetails = ({navigation, route}) => {
    ...
    const submitData = () => {
        ...
        .then(res => res.json())
        .then(data => {
            console.log(`From submit data ${damageImageUrls}`)
            console.log(`From submit ${imageUrls}`)
            console.log(data);
        }).catch(err => {
            console.log(err);
        })
    ...
    return(
        <KeyboardAvoidingView behavior="position" style={styles.root} enabled={enableShift}>
            <View >
                    ...
                    {
                        route.params?
                        <Button theme={theme} style={styles.inputStyle } mode="contained" onPress={() => {
                        updateDetails()
                        Alert.alert("Road Details updated");
                        navigation.navigate("Home");
                        }}>
                ...
                >
                    <View style={styles.modalView}>
                        <View style={styles.modalButtonView}>
                            <Button theme={theme} style={{width: "80%"}} mode="contained" onPress={() => {
